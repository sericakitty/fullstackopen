...
loginRouter.post('/', async (request, response) => {
  ...
  const passwordCorrect = user === null // if user is null, password is incorrect
    ? false
    : await bcrypt.compare(password, user.passwordHash) // compares password with user's passwordHash

  if (!(user && passwordCorrect)) { // if user or password is incorrect return error
    return response.status(401).json({
      error: 'invalid username or password'
    })
  }

  const userForToken = { // create user object for token
