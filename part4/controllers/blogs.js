...
blogsRouter.post('/', async (request, response, next) => {
  ...
  // 4.19, 4.20 - find user by id, using decodedToken
  const user = request.user; // 4.22 - get user from request object (set by userExtractor)
...
// 4.13, 4.21 - delete blog, check if user is authorized
blogsRouter.delete('/:id', async (request, response, next) => {
  ...
  try {
    ...
    if (blog.user && blog.user.toString() === user.id.toString()) {
      ...
      return response.status(403).json({ error: 'Forbidden: You do not have permission to delete this blog' }); // user unauthorized to delete
    }
  } catch (error) {
    next(error); // handle errors with next
  }
...
blogsRouter.put('/:id', async (request, response, next) => {
  ...
  const blog = { // creating new blog with updated likes
    likes: body.likes
  };
